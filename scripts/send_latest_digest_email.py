...
LEGACY_ENV_ALIASES = {
    ...
    "DIGEST_EMAIL_FROM": "MAIL_FROM",
    "DIGEST_EMAIL_TO": "MAIL_TO",
}
...
def send_digest_email(digest_path: Path, env: dict[str, str]) -> None:
    ...
    recipients = _mail_recipients(env["DIGEST_EMAIL_TO"])
    if not recipients:
        raise ValueError("DIGEST_EMAIL_TO must contain at least one recipient")

    message = EmailMessage()
    message["Subject"] = f"格密码论文日报 - {digest_date}"
    message["From"] = env["DIGEST_EMAIL_FROM"]
...
def main() -> int:
    env = normalize_env(dict(os.environ))
    ...
    print(f"sent email to {env['DIGEST_EMAIL_TO']}")
    return 0

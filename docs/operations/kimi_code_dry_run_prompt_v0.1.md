...
Operator role: lightweight emergency fallback runner/reviewer. Kimi Code is not the release owner. It must receive the full prompt every time and must not infer unsupported command availability.
...
- modify code unless explicitly authorized. If code changes appear necessary, stop and request Codex review.

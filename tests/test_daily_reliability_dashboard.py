...
def test_build_reliability_dashboard_for_non_empty_daily_run(tmp_path: Path) -> None:
    root = tmp_path
    _write_json(
...
def test_build_reliability_dashboard_marks_source_starved_when_all_red(tmp_path: Path) -> None:
    root = tmp_path
    _write_json(
...
def test_cli_runs_without_probe_and_without_network(tmp_path: Path) -> None:
    root = tmp_path
    _write_json(
            root / "data" / "2026-06-08.json",

...
def test_durable_verifier_detects_missing_markdown_json_pair(tmp_path: Path) -> None:
    _write(tmp_path / "data/2026-06-15.json", '{"records": [], "source_health": []}')
...
def test_durable_verifier_detects_invalid_json(tmp_path: Path) -> None:
    _write(tmp_path / "data/2026-06-15.json", "{not-json")
...
def test_durable_verifier_accepts_daily_weekly_monthly_artifacts(tmp_path: Path) -> None:
    ...
    _write(tmp_path / "data/2026-06-15.json", json.dumps(daily_payload))
    _write(tmp_path / "digests/2026-06-15.md", "# 2026-06-15\n\nSource health: green\n")
    _write(tmp_path / "data/weekly/2026-W25.json", json.dumps(weekly_payload))
    _write(tmp_path / "digests/weekly/2026-W25.md", "# Weekly 2026-W25\n\nSource health: green\n")
    _write(tmp_path / "data/monthly/2026-06.json", json.dumps(monthly_payload))

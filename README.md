...
Release status: v0.4.1 corrective maintenance release preparation. The immutable v0.4.0 tag remains historical evidence and points to package metadata 0.3.3. The v0.3.3 maintenance release and earlier releases remain documented; v0.2.0 remains the Research Library Interoperability Stable Release. See [CHANGELOG.md](CHANGELOG.md), [docs/releases/v0.4.1.md](docs/releases/v0.4.1.md), [docs/releases/v0.3.3.md](docs/releases/v0.3.3.md), [docs/releases/v0.3.2.md](docs/releases/v0.3.2.md), [docs/releases/v0.3.1.md](docs/releases/v0.3.1.md), [docs/releases/v0.3.0.md](docs/releases/v0.3.0.md), [docs/releases/v0.2.0.md](docs/releases/v0.2.0.md), historical [docs/releases/v0.2.0-rc1.md](docs/releases/v0.2.0-rc1.md), and [docs/release-checklist.md](docs/release-checklist.md).
...
- 中文 8 节研究日报，输出到 `digests/YYYY/daily/YYYY-MM-DD.md`。
- 结构化 JSON 输出到 `data/YYYY/daily/YYYY-MM-DD.json`，并维护 `papers.db`。
- Weekly Markdown 输出到 `digests/YYYY/weekly/YYYY-Www.md`。
- Monthly Markdown 输出到 `digests/YYYY/monthly/YYYY-MM.md`。
...
本地邮件发送脚本只读取最新的 `digests/YYYY/daily/YYYY-MM-DD.md` 正文，不会发送 `.env`、`papers.db` 或其他敏感文件内容：
...
backfill 会逐日写入 `data/YYYY/daily/YYYY-MM-DD.json` 和 `digests/YYYY/daily/YYYY-MM-DD.md`。如果已有 GitHub Actions provisional 报告，本地权威回填会在允许替换时保留旧版本快照，便于后续审计。

## 主要输出

正式日报产物：

- `digests/YYYY/daily/YYYY-MM-DD.md`：中文 Markdown 日报。
- `data/YYYY/daily/YYYY-MM-DD.json`：结构化 digest 数据。
- `papers.db`：本地论文记录数据库。
...
Artifact reads are canonical-only by default when
`LATTICE_DIGEST_ALLOW_LEGACY_FALLBACK` is unset. Temporary read-only legacy
fallback requires explicit process-scoped opt-in:

```powershell
$env:LATTICE_DIGEST_ALLOW_LEGACY_FALLBACK = "1"
```

Writers continue to use canonical year-partitioned output paths such as
`digests/YYYY/daily/YYYY-MM-DD.md` and `data/YYYY/daily/YYYY-MM-DD.json`.
...
- 不运行分类。
- 不运行测试。
- 只提交 year-partitioned `digests/`、`data/` 和 `papers.db`。
- 可补交多个历史日期。
...
- An 8-section Chinese research digest written to `digests/YYYY/daily/YYYY-MM-DD.md`.
- Structured JSON output written to `data/YYYY/daily/YYYY-MM-DD.json`, with `papers.db` as the local paper database.
...
The email script reads only the latest `digests/YYYY/daily/YYYY-MM-DD.md` body. It does not send `.env`, `papers.db`, or other sensitive files.
